## Linux Learning – Day 2

##  Date: 11/10/2025

## Goal:
Learn to manage files and directories with Linux commands.

## Commands Learned:
| Command | Description | Example |
|---------|-------------|---------|
| **touch** | Create empty files | touch test.txt |
| **mkdir** | Create directories | mkdir projects |
| **cp** | Copy files | cp file1.txt file2.txt |
| **mv** | Move or rename | mv file1.txt /home/user/ |
| **rm** | Remove files | rm file1.txt |
| **rmdir** | Remove empty directories | rmdir emptydir |
| **cat** | Display file contents | cat file.txt |
| **stat** | Show file info | stat file.txt |


## Practice Summary:
- Created, copied, moved, and deleted test files.
- Learned that `rm -r` can remove whole folders recursively.
- Created day2_practice folder with subfolders reports, backups, scripts.
- Created, copied, moved, and deleted test files.
- Learned that rm -r can remove whole folders recursively.
- Practiced viewing file contents with cat and inspecting files with stat.

## Reflection:
- I’m getting more confident using the CLI.
- Need to remember to check paths before deleting.
- Learned how to manage files and directories using commands like mkdir, touch, cp, mv, rm, and stat. Practiced creating, copying, and deleting files. Feeling more comfortable moving around Linux!

